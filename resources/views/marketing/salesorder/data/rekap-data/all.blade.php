...
<div class="col-md-12 mb-5">
    @php
        $groupedByYear = $orders->groupBy(function($order) {
    ...
    <table class="table">
        ...
        <tfoot class="bg-white">
            <tr>
                <th>{{ $orders->count('invoice_no') }}</th>
                <td class="text-center accounting subtotal">{{ number_format($orders->sum('sub_total')) }}</td>
                <td class="text-center accounting discountRp">{{ number_format($orders->sum('discount_rp')) }}</td>
                <td class="text-center accounting grandtotal">{{ number_format($orders->sum('grandtotal')) }}</td>
                <td class="text-center accounting discountRp">{{ number_format($orders->sum('sub_total')) }}</td>
                <td class="text-center accounting subtotal">{{ number_format($orders->sum('pay')) }}</td>
                <td class="text-center accounting discountRp">{{ number_format($orders->sum('due')) }}</td>
            </tr>
        </tfoot>
    </table>
</div>