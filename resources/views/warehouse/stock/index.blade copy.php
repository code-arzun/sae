                ...
                <table class="table mb-0">
                    <thead class="bg-white text-uppercase text-center">
                        <tr>
                            ...
                            @if (auth()->user()->hasAnyRole('Super Admin', 'Admin', 'Admin Gudang'))
                            <th>@sortablelink('product_ordered', 'Jumlah dipesan')</th>
                            <th>@sortablelink('stock_needed', 'Stok dibutuhkan')</th>
                            <th data-bs-toggle="tooltip" data-bs-placement="top" title="Rekap dari Total Produk yang DIPESAN">@sortablelink('Rekap Dipesan')</th>
                            <th data-bs-toggle="tooltip" data-bs-placement="top" title="Rekap dari Total Produk yang TERKIRIM">@sortablelink('Rekap Terkirim')</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <!-- Model 1 -->
                                {{-- @if ($product->product_ordered > 100)
                                        <span class="badge bg-success">{{ $product->product_ordered }}</span>
                                    @elseif ($product->product_ordered >= 50 && $product->product_ordered <= 99)
                                        <span class="badge bg-warning">{{ $product->product_ordered }}</span>
                                    @else
                                        <span class="badge bg-danger">{{ $product->product_ordered }}</span>
                                    @endif
                                <span class="ml-1">{{ $product->category->productunit->name }}</span> --}}
                                <!-- Model 2 -->
                                @if ($product->rekap_dipesan - $product->rekap_terkirim > 100)
                                    <span class="badge bg-success">{{ $product->rekap_dipesan - $product->rekap_terkirim }}</span>
                                @elseif ($product->rekap_dipesan - $product->rekap_terkirim >= 50 && $product->rekap_dipesan - $product->rekap_terkirim <= 99)
                                    <span class="badge bg-warning">{{ $product->rekap_dipesan - $product->rekap_terkirim }}</span>
                                @else
                                    <span class="badge bg-danger">{{ $product->rekap_dipesan - $product->rekap_terkirim }}</span>
                                @endif
                                <span class="ml-1">{{ $product->category->productunit->name }}</span>
                            </td>
                            <!-- Stok dibutuhkan -->
                            <td class="text-center">
                                @if ($product->stock_needed > 0)
                                    <span class="badge bg-danger">{{ $product->stock_needed }}</span>
                                @else
                                    <span class="badge bg-success">{{ $product->stock_needed }}</span>
                                @endif
                                <span class="ml-1">{{ $product->category->productunit->name }}</span>
                            </td>
                            <!-- Rekap total produk dipesan -->
                            <td class="text-center">
                                @if ($product->rekap_dipesan > 0)
                                    <span class="badge bg-danger">{{ $product->rekap_dipesan }}</span>
                                @else
                                    <span class="badge bg-success">{{ $product->rekap_dipesan }}</span>
                                @endif
                                <span class="ml-1">{{ $product->category->productunit->name }}</span>
                            </td>
                            <!-- Rekap total produk terkirim -->
                            <td class="text-center">
                                @if ($product->rekap_terkirim > 0)
                                    <span class="badge bg-danger">{{ $product->rekap_terkirim }}</span>
                                @else
                                    <span class="badge bg-success">{{ $product->rekap_terkirim }}</span>
                                @endif
                                <span class="ml-1">{{ $product->category->productunit->name }}</span>
                            </td>
                            ...
                            <td class="text-center">
                                @if ($product->rekap_selesai > 0)
                                    <span class="badge bg-danger">{{ $product->rekap_selesai }}</span>
                                @else
                                    <span class="badge bg-success">{{ $product->rekap_selesai }}</span>
                                @endif
                                <span class="ml-1">{{ $product->category->productunit->name }}</span>
                            </td>
                            @endif
