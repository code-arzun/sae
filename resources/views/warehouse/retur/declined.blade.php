...
<div class="container-fluid">
    <div class="row">
        ...
        <!-- Row & Pencarian -->
        <div class="col-lg-12">
            <form action="{{ route('retur.declined') }}" method="get">
                <div class="row align-items-start">
                    ...
                    <div class="form-group col-sm-3">
                        <select name="retur_invoice_no" id="retur_invoice_no" class="form-control"
                            ...
                            <option selected disabled>-- Pilih Kode Retur --</option>
        ...
        <div class="dt-responsive table-responsive mb-5">
            <table class="table mb-0">
                ...
                <tbody>
                    @foreach ($returs as $declined)
                    <tr>
                        <td>{{ (($returs->currentPage() * 10) - 10) + $loop->iteration  }}</td>
                        <td>
                            <span class="" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ Carbon\Carbon::parse($declined->order_date)->translatedformat('l, d F Y') }}">
                                {{ Carbon\Carbon::parse($declined->order_date)->translatedformat('d M Y') }}
                            </span>
                        </td>
                        <td>
                            <a class="badge 
                                {{ strpos($declined->invoice_no, 'ROR') !== false ? 'badge-primary' : 
                                (strpos($declined->invoice_no, 'ROH') !== false ? 'badge-danger' : 
                                (strpos($declined->invoice_no, 'RO') !== false ? 'badge-warning' : 'badge-secondary')) }}" 
                                href="{{ route('retur.Details', $declined->id) }}" 
                                data-bs-toggle="tooltip" data-bs-placement="top" title="Lihat Detail">
                                {{ $declined->invoice_no }}
                            </a>
                        </td>
                        <td>
                            <span class="" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ Carbon\Carbon::parse($declined->delivery->delivery_date)->translatedformat('l, d F Y') }}">
                                {{ Carbon\Carbon::parse($declined->delivery->delivery_date)->translatedformat('d M Y') }}
                            </span>
                        </td>
                        <td>
                            <a class="badge badge-primary" href="{{ route('do.deliveryDetails', $declined->delivery->id) }}" 
                            data-bs-toggle="tooltip" data-bs-placement="top" title="" data-original-title="Lihat Detail">{{ $declined->delivery->invoice_no }}</a></td>
                        <td><h6>{{ $declined->delivery->salesorder->customer->NamaLembaga }}</h6> {{ $declined->delivery->salesorder->customer->NamaCustomer }}</td>
                        @if (auth()->user()->hasAnyRole(['Super Admin', 'Manajer Marketing']))
                        <td>{{ $declined->delivery->salesorder->customer->employee->name }}</td>
                        @endif
                    </tr>
                    @endforeach
                </tbody>
