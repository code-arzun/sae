<!-- Rekap Dibatalkan -->
<!-- Rekap Ditolak -->
@if ($orders->where('order_status', 'Dibatalkan')->count() > 0)
    <div class="col-md">
        @php
            $groupedByYear = $orders->where('order_status', 'Dibatalkan')->groupBy(function($order) {
        ...
        <table class="table">
            ...
            <tfoot class="bg-white">
                <tr>
                    <th>{{ $orders->where('order_status', 'Dibatalkan')->count('invoice_no') }}</th>
                    <td class="text-center accounting subtotal">{{ number_format($orders->where('order_status', 'Dibatalkan')->sum('sub_total')) }}</td>
                    <td class="text-center accounting discountRp">{{ number_format($orders->where('order_status', 'Dibatalkan')->sum('discount_rp')) }}</td>
                    <td class="text-center accounting grandtotal">{{ number_format($orders->where('order_status', 'Dibatalkan')->sum('grandtotal')) }}</td>
                </tr>
            </tfoot>
        </table>
    </div>
